...
- **마우스/터치:** 탱크에서 드래그하여 각도와 파워 동시 조절 (드래그 방향 = 반대 방향 발사, 거리 = 파워)
- **키보드:** 좌우 방향키로 각도 미세 조절, 상하 방향키로 파워 조절
...
2. 방 만들기 옵션:
   ...
   - 라운드 수 (3, 5, 10)
   - 맵 테마 (랜덤, 사막, 눈, 화산)
3. 방 코드(Room Code) 공유로 친구 초대
4. 모든 플레이어 Ready 시 게임 시작
```
...
- 각 턴 시작 시 제한된 연료(fuel)로 좌우 이동 가능
- 이동 시 지형 경사를 따라 이동 (표면 추적)
- 수직 절벽은 이동 불가 (경사 각도 제한)
- Buried 상태면 이동 불가
...
│                                                          │
│              [Game World - Terrain & Tanks]               │
│                                                          │
│                                                          │
├──────────────────────────────────────────────────────────┤
│ Player 1 (You)  HP ████░░ 65  │ Angle: 45° │ Power: 70% │
│ [Shell] [Heavy] [MIRV x2] [Napalm x1]   │  [ FIRE ]    │
└──────────────────────────────────────────────────────────┘
```
