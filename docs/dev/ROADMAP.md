...
### 2.1 AI 엔진
- [x] ShotSolver - 브루트포스 궤적 시뮬레이션 (`client/src/ai/shot-solver.ts`)
- [x] AIController - 타겟 선정 + 난이도별 노이즈 (`client/src/ai/ai-controller.ts`)
- [x] 바람 보정 포함 (시뮬레이션에 자동 반영)
- [x] 지형 장애물 자동 고려 (시뮬레이션 기반)
- [x] 타겟 선정: 가장 가까운 생존 적

### 2.2 난이도 시스템
- [x] Easy: ±15° 가우시안 노이즈, 보정 없음
- [x] Normal: ±8°, 3회차 후 보정
- [x] Hard: ±3°, 2회차 후 보정
- [x] Brutal: ±1°, 즉시 보정

### 2.3 AI 턴 실행
- [x] GameManager ai_thinking / ai_charging 상태 추가
- [x] AI 사고 딜레이 0.8초
- [ ] AI 이동 결정 (향후 확장)
- [x] AI 차징 애니메이션 (파란색 게이지 바)

### 2.4 싱글플레이 설정
- [x] 게임 시작 화면 (HTML/CSS 메뉴)
- [x] 봇 수 설정 (1~7)
- [x] 봇 난이도 선택 (Easy/Normal/Hard/Brutal)
- [x] 이름: "Player" + "Bot-1", "Bot-2", ...
- [x] 랜덤 seed 지형 생성
- [x] 균등 탱크 배치 (N명)
